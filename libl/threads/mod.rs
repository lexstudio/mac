...
pub(crate) unsafe fn create(
    ...
    {
        current_sigmask =
            _rt::signal::get_sigmask().expect("failed to obtain sigprocmask for caller");
    }
...
unsafe extern "C" fn new_thread_shim(
    ...
    if let Some(tcb) = tcb.as_mut() {
        ...
        {
            tcb.activate(_rt::proc::FdGuard::new(tid.thread_fd));
            _rt::signal::setup_sighandler(&tcb.os_specific, false);
        }
    }
